...
$(function() {
	...
	$.ajax({
		type: "POST",
		url: "index.php?control=search&action=getCourse",
		data: {
			page: 1 , 
			keyword: keyword
		},
		success: function(data) {
			...
			$(".course_list .page li").eq(0).addClass("active");
		}
	}).then(function() {
		//获取页码
		$.get("index.php?control=search&action=getPage", {cur_ten: 1, info: "course", keyword: keyword}, function(rtn) {
	...
	$(".page").on("click", ".next", function(e) {
		var this_elm = $(e.target);
		var page = Number($("#cur_ten").val()) * 10 * 2 + 1;
		...
		$.ajax({
			...
			$.get("index.php?control=search&action=getPage", {cur_ten: cur_ten, info: "course", keyword: keyword}, function(rtn) {
				var rtnObj = JSON.parse(rtn);
				$(".page").html(rtnObj.html);
				$("#cur_ten").val(rtnObj.cur_ten);
	...
	$(".page").on("click", ".prev", function(e) {
		var this_elm = $(e.target);
		var page = (Number($("#cur_ten").val()) - 2) * 10 * 2 + 1;
		var cur_ten = Number($("#cur_ten").val()) - 1;
		$.ajax({
			type: "POST",
			url: "index.php?control=search&action=getTeacher",
